...
<body>
    ...
    <p><strong>Product Price:</strong> N{{ number_format($order->product_price) }}</p>
    <p><strong>Delivery Location:</strong> {{ $order->delivery_city }}, {{ $order->delivery_state }}</p>
    <p><strong>Delivery Price:</strong> N{{ number_format($order->delivery_price) }}</p>
    <p><strong>Total Price:</strong> N{{ number_format($order->total_price) }}</p>
